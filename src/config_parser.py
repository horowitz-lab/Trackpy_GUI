...
def get_config():
    ...
    config_dict["rb_gallery_folder"] = os.path.join(
        os.path.dirname(__file__), "rb_gallery"
    )
...
def save_detection_params(params):
    ...
    config["Detection"]["invert"] = (
        "true" if bool(params.get("invert", False)) else "false"
    )
...
def get_linking_params():
    ...
    if config.has_section("Linking"):
        ...
        if "min_trajectory_length" in section:
            try:
                params["min_trajectory_length"] = int(
                    section.get("min_trajectory_length")
                )
            except Exception:
                pass
        if "fps" in section:
...
def save_linking_params(params):
    ...
    config["Linking"]["min_trajectory_length"] = str(
